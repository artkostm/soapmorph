...
import com.artkostm.data.soap.client.schema.{DataF, SchemaF, Xsd}
...
class SoapSttpClient(config: SoapClientConfig,
                     ...
                     wsdlDefStore: Ref[Map[String, WsdlDefinition]])(backend: SttpClient.Service)
  extends SoapClient.Service {
  import SoapSttpClient._

  override def readData(service: String, action: String, xmlElement: String, namespace: NamespaceBinding)(
    data: Map[String, List[String]]
  ): Task[Stream[Throwable, Map[String, String]]] =
    call(uri"${config.baseUrl}/$service", action, namespace)(data)
      .map(xmlStreamToMapStream(xmlElement)(_))

  override def readData(action: String, wsdlUrl: String)(
    data: Map[String, List[String]]
  ): Task[GenericStreamingResponse] =
    for {
      wsdlStore <- wsdlDefStore.get
      definition <- if (wsdlStore.contains(wsdlUrl)) Task.effect(wsdlStore.apply(wsdlUrl))
                   else
                     wsdlLoader
                       .definitionFor(wsdlUrl)
                       .flatMap(d => wsdlDefStore.update(old => old + (wsdlUrl -> d)) *> Task.succeed(d))
      operation <- definition.service.port.binding.portType.operations.find(_.name == action) match {
                    case Some(op) => Task.succeed(op)
                    case _ =>
                      Task.fail(
                        new RuntimeException(
                          s"Cannot find action: $action, possible actions ${definition.service.port.binding.portType.operations
                            .mkString("[", ", ", "]")}"
                        )
                      )
                  }
      Some((topElementLabel, dataSchema)) = SchemaF.getHigherLevelFieldType(
        definition.types(new QName(definition.service.targetNamespace, operation.outputMessage))
      )
    ...
    } yield (dataSchema, xmlStreamToDataStream(topElementLabel, xmlValidator)(entityStream))

  private def call(url: Uri, action: String, namespace: NamespaceBinding)(
    data: Map[String, List[String]]
...
object SoapSttpClient extends BasicWriters with SpecialWriters{

  protected def payload(rootTag: String, prefix: String)(data: Map[String, List[String]]): Elem =
    <x>{ data.map {
      case (element, values) => Xml.toXml(values, <x/>.copy(label = element, prefix = prefix))
    } }</x>.copy(label = rootTag, prefix = prefix)

  protected def security(user: String, password: String)(ns: NamespaceBinding): Elem = {
    val security =
  ...
  protected def envelope(body: NodeSeq = NodeSeq.Empty,
                       header: NodeSeq = NodeSeq.Empty)(ns: NamespaceBinding): NodeSeq = {
    val env =
      <soapenv:Envelope>
  ...
  protected val WsseSn: NamespaceBinding = NamespaceBinding(
    "wsse",
    "http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-secext-1.0.xsd",
    xml.TopScope
  )

  protected def xmlStreamToMapStream(
    xmlElementName: String
  )(xmlByteStream: Stream[Throwable, Byte]): Stream[Throwable, Map[String, String]] = {
    ...
    anchorElements.map { e =>
      ...
      val c = complex.headOption.map { first =>
        <x>{ complex.reduceLeftOption[NodeSeq]((n1, n2) => n1 ++ n2).getOrElse(Seq.empty) }</x>.copy(
          label = first.label + "s"
        )
  ...
  protected def xmlStreamToDataStream(
    xmlElementName: String,
    validator: Xsd.FixedDataRule
  )(xmlByteStream: Stream[Throwable, Byte]): Stream[Throwable, Fix[DataF]] = {
    val anchorElementExtractor: XmlElementExtractor[Elem] =
      XmlElementExtractor.filterElementsByName(xmlElementName)
    ...
    anchorElements.flatMap { e =>
      validator
        ...
        .fold(
          errors =>
            Stream.fail(new RuntimeException(
              s"Error while validating streaming data labeled ${e.label}: \n${errors.mkString(",\n")}\n\n$e"
            )),
          Stream.succeed(_)
        )
    }.provideLayer(Blocking.live)
