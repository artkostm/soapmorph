...
object DataF {
  ...
  def struct(fields: Map[String, Fix[DataF]]): Fix[DataF] =
    GStruct(fields).fix[DataF]
  ...
  implicit class FixedData(d: Fix[DataF]) {
    ...
    def get(fieldName: String): Option[Any] = {
      @tailrec def search(dd: Option[Fix[DataF]]): Option[Fix[DataF]] = dd match {
        case Some(Fix(GStruct(fields)))  => search(fields.get(fieldName))
        case Some(f @ Fix(_: GValue[_])) => Some(f)
        case _                           => None
      }

      search(Some(d)).flatMap(Fix.un(_) match {
        case field: GValue[_] => Option(field.value)
        case _                => None
      })
    }

    def getField(fieldName: String): Fix[DataF] = {
      @tailrec def search(dd: Option[Fix[DataF]]): Option[Fix[DataF]] = dd match {
        case Some(Fix(GStruct(fields)))  => search(fields.get(fieldName))
        case Some(f @ Fix(_: GValue[_])) => Some(f)
        case _                           => None
      ...
      case (structF @ Fix(StructF(fields, _)), Fix(GStruct(values))) =>
        AttrF
          .apply[DataF, Fix[SchemaF], (Fix[SchemaF], Fix[DataF])](
            (structF, GStruct(values.filter(p => fields.contains(p._1)).map {
